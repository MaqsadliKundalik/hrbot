...
async def ortga(message: Message, state: FSMContext):
    ...
    match await state.get_state():
        ...
        case AdminSubjectStates.add_sertifikat:
            ...
            await message.answer("Ortga qaytildi!", reply_markup=sertifikatlar_lst_btn([s.name for s in await Sertificates.filter(subject=state_data['selected_fan'])], is_admin=True))
        ...
        case AdminSubjectStates.add_ball:
            ...
            await message.answer("Ortga qaytildi!", reply_markup=sertifikat_balls_lst_btn(state_data['selected_sertificate'].ball_list, is_admin=True, is_new=False))
        case AdminSubjectStates.delete_ball:
            await state.set_state(AdminSubjectStates.view_balls)
            await message.answer("Ortga qaytildi!", reply_markup=sertifikat_balls_lst_btn(state_data['selected_sertificate'].ball_list, is_admin=True, is_new=False))
        case AdminSubjectStates.update_quiz:
            await state.set_state(AdminSubjectStates.select_sertifikat)
            await message.answer("Ortga qaytildi!", reply_markup=sertifikatlar_lst_btn([s.name for s in await Sertificates.filter(subject=state_data['selected_subject'])], is_admin=True))
...
async def select_fan(message: Message, state: FSMContext):
    ...
    await state.update_data(selected_fan=subject)
...
async def update_test_file(message: Message, state: FSMContext, bot: Bot):
    state_data = await state.get_data()
    subject = state_data['selected_fan']
...
async def update_vacancy_text(message: Message, state: FSMContext):
    state_data = await state.get_data()
    subject = state_data['selected_subject']
    vacancy = await VacanciesText.filter(name=subject.name).first()
    if vacancy:
        await state.update_data(selected_vacancy=vacancy)
...
async def update_vacancy_text(message: Message, state: FSMContext):
    state_data = await state.get_data()
    vacancy = state_data['selected_vacancy']
    await vacancy.update(text=message.text)
    await state.set_state(AdminSubjectStates.select_sertifikat)
    await message.answer("Vakansiya matni yangilandi!", reply_markup=sertifikatlar_lst_btn([s.name for s in await Sertificates.filter(subject=state_data['selected_subject'])], is_admin=True))
...
async def add_sertifikat(message: Message, state: FSMContext):
    ...
    await state.update_data(selected_sertificate=sertificate)
...
async def add_sertifikat_balls(message: Message, state: FSMContext):
    state_data = await state.get_data()
    sertificate = state_data['selected_sertificate']
    if not sertificate.ball_list:
        await message.answer("Bu sertifikatga hali ball qo'shilmadi!")
        return
    await message.answer(f"\"{sertificate.name}\" sertifikatiga ballar qo'shildi!", reply_markup=sertifikatlar_lst_btn([s.name for s in await Sertificates.filter(subject=state_data['selected_fan'])], is_admin=True))
    await state.set_state(AdminSubjectStates.select_sertifikat)
...
async def delete_sertificate(message: Message, state: FSMContext):
    ...
    await state.set_state(AdminSubjectStates.select_sertifikat)
    await message.answer("Sertifikat o'chirildi!", reply_markup=sertifikatlar_lst_btn([s.name for s in await Sertificates.filter(subject=state_data['selected_fan'])], is_admin=True))
...
async def delete_ball(message: Message, state: FSMContext):
    ...
    state_data = await state.get_data()
    await message.answer("O'chiriladigan ballni tanlang", reply_markup=sertifikat_balls_lst_btn(state_data['selected_sertificate'].ball_list, is_admin=False, is_new=False))
