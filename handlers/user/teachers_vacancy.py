...
async def teachers_vacancy_back(message: Message, state: FSMContext):
    ...
    match await state.get_state():
        ...
        case TeachersVacancyState.sertificate_ball:
            await message.answer("Sertifikatingizni tanlang.", reply_markup=sertifikatlar_lst_btn([sert.name for sert in await Sertificates.filter(subject=state_data["subject"])], False))
            await state.set_state(TeachersVacancyState.sertificate_name)
        case TeachersVacancyState.sertificate_file:
            await message.answer("Sertifikat bo'yicha overall ballingizni tanlang.", reply_markup=sertifikat_balls_lst_btn([ball for ball in state_data["sertificate"].ball_list], False))
...
async def select_has_sertificate(message: Message, state: FSMContext):
    ...
    if message.text == "Ha":
        ...
        quiz = await Quizs.get_or_none(subject=state_data["subject"])
...
async def select_sertificate_name(message: Message, state: FSMContext):
    ...
    if not sertificate:
        ...
        return
    await state.update_data(sertificate=sertificate)
...
async def select_sertificate_file(message: Message, state: FSMContext, bot: Bot):
    ...
    sertificates.append({"name": state_data["sertificate"].name, "ball": state_data["sertificate_ball"], "file_id": message.document.file_id})
...
async def start_quizs(message_id: int, state: FSMContext, bot: Bot, chat_id: int):
    state_data = await state.get_data()
    quizs = await Quizs.filter(subject=state_data["subject"]).first()
    await state.update_data(quizs=quizs, quiz_index=0, correct_answers=0, start_time=datetime.now(timezone.utc))
    ...
    state_data = await state.get_data()
    while datetime.now(timezone.utc) - state_data["start_time"] < timedelta(seconds=60):
        await sleep(60)
    ...
    if state_now == TeachersVacancyState.quizs:
        ...
        await bot.send_message(chat_id, "<b>Testga ajratilgan vaqt tugadi!</b>\n\nTo'g'ri javoblar: {}\nJavoblar soni: {} ta\nSavollar soni: {} ta".format(state_data["correct_answers"], state_data["quiz_index"] + 1, len(quizs.quizs)), parse_mode="HTML")
        ...
        if state_data["correct_answers"] >= len(quizs.quizs) * 0.8:
...
async def poll_answer_handler(answer: PollAnswer, state: FSMContext, bot: Bot):
    state_data = await state.get_data()
    quizs = state_data["quizs"]
    quiz_index = state_data["quiz_index"]
    
    if datetime.now(timezone.utc) - state_data["start_time"] > timedelta(seconds=60):
        return

    if answer.option_ids[0] == quizs.quizs[quiz_index]["answer"] - 1:
    ...
    if quiz_index == len(quizs.quizs) - 1:
        ...
        await bot.send_poll(
            ...
            question=quizs.quizs[quiz_index + 1]["question"],
            options=[
                *quizs.quizs[quiz_index + 1]["options"],
            ],
            type="quiz",
            correct_option_id=quizs.quizs[quiz_index + 1]["answer"] - 1,
            is_anonymous=False,
        )
...
async def select_salary(message: Message, state: FSMContext):
    ...
    await TeacherResume.create(
        subject=state_data['subject'].name,
